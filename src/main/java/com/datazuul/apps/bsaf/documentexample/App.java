 ...
 */
package com.datazuul.apps.bsaf.documentexample;
...
/**
 * This is a very simple example of a SingleFrameApplication that loads and saves a single text document. Although it
 * does not possess all of the usual trappings of a single-document app, like versioning or support for undo/redo, it
 * does serve to highlight how to use actions, resources, and tasks.
 *
 * <p>
 ...
 * </dl>
 * These properties are updated when the user interacts with the application. They can be used as binding sources, to
 * monitor the application's state.
 *
 * <p>
 * The application is {@link Application#launch launched} in the main method on the "main" thread. All the work of
 * actually constructing, {@link #initialize intializing}, and {@link #startup starting} the application actually
 * happens on the EDT.
 *
 * <p>
 ...
 *
 * <p>
 * This application defines a small set of actions for opening and saving files: {@link #open open}, {@link #save save},
 * and {@link #saveAs saveAs}. It inherits {@code cut/copy/paste/delete} ProxyActions from the {@code Application}
 * class. The ProxyActions perform their action not on the component they're bound to (menu items and toolbar buttons),
 * but on the component that currently has the keyboard focus. Their enabled state tracks the selection value of the
 * component with the keyboard focus, as well as the contents of the system clipboard.
 *
 * <p>
 * The action code that reads and writes files, runs asynchronously on background threads. The
 * {@link #open open}, {@link #save save}, and {@link #saveAs saveAs} actions all return a Task object which
 * encapsulates the work that will be done on a background thread. The {@link #showAboutBox showAboutBox} and
 * {@link #closeAboutBox closeAboutBox} actions do their work synchronously.
 *
 * <p>
 * <strong>Warning:</strong> this application is intended as a simple example, not as a robust text editor. Read it,
 * don't use it.
 *
...
public class App extends SingleFrameApplication {
     ...
     * <p>
     * The file is loaded on a worker thread because we don't want to block the EDT while the file system is accessed.
     * To do that, this Action method returns a new LoadFileTask instance, if the user confirms selection of a file. The
     * task is executed when the "open" Action's actionPerformed method runs. The LoadFileTask is responsible for
     * updating the GUI after it has successfully completed loading the file.
     *
    ...
    private class SaveFileTask extends SaveTextFileTask {
        ...
        SaveFileTask(File file) {
            super(App.this, file, textArea.getText());
        }
     ...
     * <p>
     * The text is written to the file on a worker thread because we don't want to block the EDT while the file system
     * is accessed. To do that, this Action method returns a new SaveFileTask instance. The task is executed when the
     * "save" Action's actionPerformed method runs. The SaveFileTask is responsible for updating the GUI after it has
     * successfully completed saving the file.
     *
     *
     * @see #getFile
     */
     ...
     * <p>
     * This action is nearly identical to {@link #open open}. In this case, if the user chooses a file, a
     * {@code SaveFileTask} is returned. Note that the selected file only becomes the value of the {@code file} property
    ...
    /**
     * A Task that saves a text String to a file. The file is not appended to, its contents are replaced by the String.
     */
    private static class SaveTextFileTask extends Task<Void, Void> {
        ...
        /**
         * Writes the {@code text} to the specified {@code file}. The implementation is conservative: the {@code text}
         * is initially written to ${file}.tmp, then the original file is renamed ${file}.bak, and finally the temporary
         * file is renamed to ${file}. The Task's {@code progress} property is updated as the text is written.
         * <p>
         * If this Task is cancelled before writing the temporary file has been completed, ${file.tmp} is deleted.
         * <p>
         * The conservative algorithm for saving to a file was lifted from the FileSaver class described by Ian Darwin
         * here:
    ...
    private static class LoadTextFileTask extends Task<String, Void> {
        ...
        /**
         * Load the file into a String and return it. The {@code progress} property is updated as the file is loaded.
         * <p>
         * If this task is cancelled before the entire file has been read, null is returned.
         *
